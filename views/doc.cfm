	...
	<div class="jumbotron">
		<div class="container">
			...
			<cfif StructKeyExists(data, "discouraged") AND Len(data.discouraged)>
				<div class="alert alert-danger">
					<h4><span class="glyphicon glyphicon-warning-sign"></span> Discouraged: #autoLink(encodeForHTML(parseMarkdown(data.discouraged)))#</h4>
				</div>
			</cfif>
		</div>
	</div>
								...
								<cfif StructKeyExists(data.engines[i], "deprecated") AND Len(data.engines[i].deprecated)>
									<div class="alert alert-danger">
										...
										<cfif StructKeyExists(data.engines[i], "notes") AND Len(data.engines[i].notes)>
											#autoLink(encodeForHTML(parseMarkdown(data.engines[i].notes)))#
										</cfif>
									</div>
								<cfelse>
									<div class="alert alert-warning">
